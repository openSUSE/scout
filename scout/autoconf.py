...
class ScoutModule(object):
    ...
    def query(cls, repo, term):
        ...
        r = db.query('SELECT package, m4, macro FROM macros LEFT JOIN m4s ON macros.id_m4=m4s.id_m4 LEFT JOIN packages ON m4s.id_pkg=packages.id_pkg WHERE macro LIKE ?', '%%%s%%' % term)
        if r == None:
            return None
